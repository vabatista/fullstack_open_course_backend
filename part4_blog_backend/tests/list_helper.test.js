...
describe('total likes', () => {
	...
	test('when receive a long list of entries', () => {
		const result = listHelper.totalLikes(blogs)
		expect(result).toBe(36)
	})

	test('when not passing array should return 0', () => {
		const result = listHelper.totalLikes('a')
		expect(result).toBe(0)
	})

	test('when passing empty array should return 0', () => {
...
describe('Favorite blog', () => {
	...
	test('when receive a long list of entries', () => {
		...
		const favorite = { _id: '5a422b3a1b54a676234d17f9', title: 'Canonical string reduction', author: 'Edsger W. Dijkstra', 
			url: 'http://www.cs.utexas.edu/~EWD/transcriptions/EWD08xx/EWD808.html', likes: 12, __v: 0 }
		expect(result).toEqual(favorite)
	})

	test('when not passing array should return null', () => {
		const result = listHelper.favoriteBlog('a')
		expect(result).toBe(null)
	})

	test('when passing empty array should return null', () => {
...
describe('Most blogs', () => {
	...
	test ('when receive a list with one entry', () => {
